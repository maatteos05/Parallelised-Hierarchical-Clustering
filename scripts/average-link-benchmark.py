"""
benchmark.py — Benchmark average-link HAC implementations across dataset sizes and thread counts.

Run from the repo ROOT:
    python scripts/benchmark.py

Outputs:
    data/benchmarks/average_link_results.csv   — raw timing data
    data/benchmarks/average_link_speedup.csv   — speedup relative to sequential baseline
...
RESULTS_CSV     = BENCHMARKS_DIR / "average_link_results.csv"
SPEEDUP_CSV     = BENCHMARKS_DIR / "average_link_speedup.csv"
...
def build_cmd(algo, input_csv, output_csv, n_threads=None):
    if algo == "seq":
        ...
        return [str(BINARIES["naive"]), str(input_csv), str(output_csv),
                str(n_threads)]
    elif algo == "ppop":
        return [str(BINARIES["ppop"]), str(input_csv), str(output_csv),
                str(n_threads)]
...
def parse_time_ms(stdout):
    """Extract time from a line containing 'done in X ms'."""
    match = re.search(r"done in\s+([\d.]+)\s*ms", stdout)
...
def main():
    ...
    tmp_out = BENCHMARKS_DIR / "average_link_bench_tmp.csv"

    print("=" * 60)
    print("HAC Average-Link Benchmark")
    print("=" * 60)
...
def plot_speedup(df_sp):
    ...
    for algo in df_sp["algo"].unique():
        ...
        fig.savefig(PLOTS_DIR / f"average_link_speedup_{algo}.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
    ...
    if not df_sp.empty:
        ...
        ax.legend()
        ax.grid(True, alpha=0.3)
        fig.savefig(PLOTS_DIR / "average_link_speedup_vs_size.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
...
def plot_time_vs_size(df):
    ...
    ax.set_title("Wall-clock time vs dataset size")
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.savefig(PLOTS_DIR / "average_link_time_vs_size.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
